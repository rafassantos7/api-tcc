...
export default function Habitos() {
  ...
  const colorOptions = ["#8b5cf6", "#ec4899", "#3b82f6", "#10b981", "#f59e0b", "#ef4444", "#6366f1"];
      ...
      <main className={`${styles.conteudo} ${isOpen && !isMobile ? styles["sidebar-open"] : ""}`}>
        <header className={styles["habitos-header"]}>
          ...
          <div className={styles["header-actions"]}>
            ...
            >
              {isAdding || editingHabit ? "Cancelar" : "+ Novo Hábito"}
            </button>
          </div>
        </header>

        {/* Formulário para adicionar/editar hábito */}
        {(isAdding || editingHabit) && (
          <div className={styles["habit-form"]}>
            <div className={styles["form-header"]}>
              <h3>{editingHabit ? "Editar Hábito" : "Criar Novo Hábito"}</h3>
            </div>
              ...
              <div className={styles["form-group"]}>
                ...
                <div className={styles["frequency-options"]}>
                  {frequencyOptions.map(option => {
                    const currentFrequency = editingHabit ? editingHabit.frequency : newHabit.frequency;
                    return (
                      <button
                        key={option.value}
                        type="button"
                        className={`${styles["frequency-btn"]} ${currentFrequency === option.value ? styles["active"] : ""}`}
                        onClick={() => editingHabit
                          ? setEditingHabit({...editingHabit, frequency: option.value})
                          : setNewHabit({...newHabit, frequency: option.value})
                        }
                      >
              ...
              <div className={styles["form-group"]}>
                ...
                <div className={styles["goal-slider"]}>
                  <input
                    ...
                    onChange={(e) => editingHabit
                      ? setEditingHabit({...editingHabit, goal: parseInt(e.target.value)})
                      : setNewHabit({...newHabit, goal: parseInt(e.target.value)})
                    }
                  />
                  <span className={styles["goal-value"]}>
                    {editingHabit ? editingHabit.goal : newHabit.goal} dias
                  </span>
                </div>
              </div>
                  ...
                  {iconOptions.slice(0, 10).map(icon => {
                    ...
                    return (
                      <button
                        key={icon}
                        type="button"
                        className={`${styles["icon-option"]} ${currentIcon === icon ? styles["selected"] : ""}`}
                        onClick={() => editingHabit
              ...
              <div className={styles["form-group"]}>
                ...
                <div className={styles["color-picker"]}>
                  {colorOptions.map(color => {
                    const currentColor = editingHabit ? editingHabit.color : newHabit.color;
                    return (
                      <button
                        key={color}
                        type="button"
                        className={`${styles["color-option"]} ${currentColor === color ? styles["selected"] : ""}`}
            ...
            <div className={styles["form-actions"]}>
              ...
              >
                {editingHabit ? "Atualizar Hábito" : "Criar Hábito"}
              </button>
            </div>
          </div>
        )}
          ...
          {habits.length === 0 ? (
            ...
            <>
                  ...
                  return (
                    ...
                    >
                      <div className={styles["habit-header"]}>
                        <div className={styles["habit-icon"]} style={{ backgroundColor: `${habit.color}20` }}>
                          {habit.icon}
                        </div>
                        ...
                        <div className={styles["habit-actions"]}>
                          <button
                            className={styles["edit-habit-btn"]}
                      ...
                      <div className={styles["habit-completion"]}>
                        <button
              ...
              <div className={styles["habits-calendar"]}>
                <h3>Histórico Recente (Últimos 7 dias)</h3>
